...
T = {
    ...
    # ---- milestone payments on the pricing page ----
    'A one-off fee to build it, then a monthly fee to run it. The build fee can be paid in three stages — 40% to start, 30% when you approve the design, 30% when it goes live — so you are never far ahead of the work. The monthly covers hosting, the database, the assistant, backups and the changes you ask for. No VAT is charged.':
        ('Ένα εφάπαξ κόστος κατασκευής και μετά μηνιαία συνδρομή. Το κόστος κατασκευής μπορεί να πληρωθεί σε τρεις δόσεις — 40% στην αρχή, 30% όταν εγκρίνετε το σχεδιασμό, 30% όταν βγει στον αέρα — ώστε να μην προπληρώνετε ποτέ πολύ μπροστά. Η συνδρομή καλύπτει τη φιλοξενία, τη βάση δεδομένων, τον βοηθό, τα αντίγραφα ασφαλείας και τις αλλαγές που ζητάτε. Δεν χρεώνεται ΦΠΑ.',
         'Разовая плата за сборку, затем ежемесячная за работу. Плату за сборку можно внести в три этапа — 40% на старте, 30% после утверждения дизайна, 30% при запуске — чтобы вы никогда не платили сильно вперёд. Ежемесячная покрывает хостинг, базу данных, ассистента, резервные копии и правки, о которых вы просите. НДС не начисляется.'),

    # ---- the trading identity in the footer ----
    'Mark Saade, sole trader &middot; Livadia, Larnaca, Cyprus &middot; not registered for VAT':
        ('Mark Saade, ατομική επιχείρηση &middot; Λιβάδια, Λάρνακα, Κύπρος &middot; χωρίς εγγραφή στο ΦΠΑ',
         'Mark Saade, индивидуальный предприниматель &middot; Ливадия, Ларнака, Кипр &middot; не зарегистрирован как плательщик НДС'),
